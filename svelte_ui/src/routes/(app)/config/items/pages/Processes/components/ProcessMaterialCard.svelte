<script lang="ts">
  ...
  import { deleteProcessMaterial } from "../api/processes.api";
  ...
  const handleDeleteProcessMaterial = () => {
    const promise = deleteProcessMaterial(materialId, processId);
  ...
  const handleIncreaseQtd = () => {
    updateProcessMaterial(materialId, Number(qtd) + 1, processId)
      .catch((err) => {
